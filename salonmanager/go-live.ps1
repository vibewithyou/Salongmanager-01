...
if (!(Get-Command php -ErrorAction SilentlyContinue)) { 
    Write-Host "⚠️  PHP not in PATH - will use Docker flow" -ForegroundColor Yellow 
}
if (!(Get-Command composer -ErrorAction SilentlyContinue)) { 
    Write-Host "⚠️  Composer not in PATH - will use Docker flow" -ForegroundColor Yellow 
}

# Step 2: Docker Stack Setup
Write-Host "`n🐳 Step 2: Docker Stack Setup" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "`n⚙️  Step 3: Laravel Setup" -ForegroundColor Yellow
    ...
    Write-Host "`n🏥 Step 5: Health Checks" -ForegroundColor Yellow
    ...
    Write-Host "`n🔗 Step 6: Webhook Testing" -ForegroundColor Yellow
    ...
    Write-Host "`n🎉 Go Live Process Completed!" -ForegroundColor Green
    Write-Host "✅ All systems are ready for production" -ForegroundColor Green
